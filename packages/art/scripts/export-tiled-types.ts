...
const dumpObjectAsError = (obj: any) => console.error(JSON.stringify(obj, null, 2));
...
const checkTile = (tile: xlmJs.ElementCompact): boolean => {
  ...
  const index = indexUnparsed != undefined && typeof indexUnparsed === "string" ? parseInt(indexUnparsed) : undefined;
  if (index == undefined) {
    return false;
  }
...
const parseTile = (tile: xlmJs.ElementCompact) => {
  const indexUnparsed = tile._attributes!.id!;
  const index = indexUnparsed && typeof indexUnparsed === "string" ? parseInt(indexUnparsed) : undefined;
  if (index == undefined) {
  ...
  const frameNumber = frameNumberUnparsed != undefined ? parseInt(frameNumberUnparsed) : undefined;
...
const buildAnimatedTileList = (tileObjects: Tile[]) => {
  ...
  for (const [k, v] of Object.entries(tilesWithSameNameAndFrameNumber)) {
    animatedTiles.push({
      frames: v.sort((a, b) => a.frameNumber! - b.frameNumber!).map((t) => t.index),
      name: k,
    });
  }
...
// packages/client/src/layers/Renderer/Phaser/tilesets
