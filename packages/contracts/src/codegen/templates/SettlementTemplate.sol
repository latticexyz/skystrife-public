...
import { Combat, StructureType, Untraversable, Capturable, Charger, ChargeCap, Factory } from "base/codegen/index.sol";
...
function SettlementTemplate() {
  ...
  (staticData, encodedLengths, dynamicData) = Combat.encode(250000, 250000, 0, 0, 0, 0, CombatArchetypes(uint8(10)));
  ...
  (staticData, encodedLengths, dynamicData) = Charger.encode(0);
  staticDatas[4] = staticData;
  encodedLengthss[4] = EncodedLengths.unwrap(encodedLengths);
  dynamicDatas[4] = dynamicData;

  (staticData, encodedLengths, dynamicData) = ChargeCap.encode(0, 0);
