...
function ArcherTemplate() {
  ...
  (staticData, encodedLengths, dynamicData) = Combat.encode(100000, 100000, 0, 50000, 0, -100);
  ...
  (staticData, encodedLengths, dynamicData) = CombatArchetype.encode(CombatArchetypes(uint8(7)));
  staticDatas[2] = staticData;
  encodedLengthss[2] = PackedCounter.unwrap(encodedLengths);
  dynamicDatas[2] = dynamicData;

  (staticData, encodedLengths, dynamicData) = StaminaOnKill.encode(100);
