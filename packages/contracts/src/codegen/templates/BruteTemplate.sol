...
bytes32 constant templateId = "Brute";
bytes32 constant BruteTemplateId = templateId;
uint256 constant LENGTH = 8;

function BruteTemplate() {
  ...
  (staticData, encodedLengths, dynamicData) = Combat.encode(300000, 300000, 0, 50000, 0, -30);
  staticDatas[0] = staticData;
  encodedLengthss[0] = PackedCounter.unwrap(encodedLengths);
  dynamicDatas[0] = dynamicData;

  (staticData, encodedLengths, dynamicData) = UnitType.encode(UnitTypes(uint8(10)));
  staticDatas[1] = staticData;
  encodedLengthss[1] = PackedCounter.unwrap(encodedLengths);
  dynamicDatas[1] = dynamicData;

  (staticData, encodedLengths, dynamicData) = CombatArchetype.encode(CombatArchetypes(uint8(13)));
