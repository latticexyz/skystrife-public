...
ResourceId constant _tableId = ResourceId.wrap(
  bytes32(abi.encodePacked(RESOURCE_TABLE, bytes14(""), bytes16("LevelPosition")))
);
ResourceId constant LevelPositionTableId = _tableId;
...
library LevelPosition {
   ...
   */
  function get(bytes32 levelId, uint256 index) internal view returns (int32 x, int32 y) {
    bytes32[] memory _keyTuple = new bytes32[](2);
    _keyTuple[0] = levelId;
    _keyTuple[1] = bytes32(uint256(index));
   ...
   * @notice Get the full data.
   */
  function _get(bytes32 levelId, uint256 index) internal view returns (int32 x, int32 y) {
    bytes32[] memory _keyTuple = new bytes32[](2);
    _keyTuple[0] = levelId;
    _keyTuple[1] = bytes32(uint256(index));
